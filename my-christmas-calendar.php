...
Plugin URI: https://adventcalendar.com
Description: Embed your Advent calendar from adventcalendar.com in Wordpress.
Version: 1.1.6
Author: Stian B Pedersen
Author URI: https://adventcalendar.com
*/
...
function mcac_add_calendar($atts) {
	...
	if( strlen($mcac_subdomain) > 1 ) {
		...
		return "<iframe class='mcac_{$mcac_subdomain}' frameborder='0' style='width:{$c_width};height:{$c_height};border:{$c_border} solid {$c_bordercolor}' marginwidth='0' marginheight='0' src='//{$mcac_subdomain}'></iframe>";
	}else{
		return '';	
	}
}
